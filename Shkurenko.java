...
public class Shkurenko {
	...
	public static void main(String[] args) {
		...
		*/
		...
		if(film > 0 && skorost > 0){
			long filmb =(long) film * 1024*1024*1024;// количество бит в фильме
		...
		*/
		...
		int b = a /60/60;//час
		int min = (a - (b * 60 * 60))/60;//минуты
		int secf = a - ((b * 60 * 60) + (min * 60));
		...
		Scanner sc = new Scanner(System.in);
		System.out.println("Введите обьем фильма для переноса");
		double razm = sc.nextDouble();
		if (razm > 0){
			double a = razm*1000;// из Гбайт сделал Мбайт
			a = a /1.44;
			a = new BigDecimal(a).setScale(0, RoundingMode.UP).doubleValue();
			System.out.println("для переноса фильма потребуется " + a + " дискет размером 1.44 Мбайт");
		}
		System.out.println("Размер фильма должен быть положительным");
		...
		int mes = sc.nextInt();
		System.out.println("Введите процентную ставку за год");
		double proc = sc.nextDouble();
		if (grn > 0 && mes > 0 && proc > 0){
			double a = (grn * proc) / 100;
			double b = (a * (mes *30))/365;
			double c = b + grn;
			c = new BigDecimal(c).setScale(2, RoundingMode.UP).doubleValue();
			System.out.println("Пользователь получит по прошествию " + mes + " месяцев " + c + " гривен");
		}
		System.out.println("Цифры не могут быть отрицательными")
		
		// 17. Рассчитать площадь поверхности Земли.
		int a = 148940000;//км² суша (29,2 %)
		int b = 361132000;//км² вода (70,8 %)
		int c = a + b;
		System.out.println("Площадь поверхности земли = " + a + " площадь поверхности суши + " + b + " площадь поверхности воды и будет =" + c + " км квадратных");
