...
public class Shkurebko_6 {
	...
	public static void main(String[] args) {
		...
		//16.	Реализовать игровую программу «Однорукий бандит» (игровой автомат). В начале игры пользователю выдаётся 5000 кредитов. 
		...
		System.out.println("sorry u luse = " + (score -10)+ " your score");
			*/
		//17.	Королю нужно убить дракона, но средства его казны – ограничены.  Нужно создать программу, которая поможет рассчитать 
		//минимальное количество копейщиков, которое необходимо, чтобы убить дракона. Данные очков здоровья и атаки дракона и одного
		//копейщика вводятся с клавиатуры. Защиту, удачу, мораль, меткость и т.п. не учитывать. Копейщики наносят удар первыми 
		//(общий нанесённый урон – это сумма атак всех живых копейщиков). Если атака дракона превышает значение очков здоровья копейщика 
		//(например, у копейщика хп 10, а атака дракона - 15),  то копейщик погибает, а оставшийся урон действует на оставшихся в живых 
		//копейщиков. Например, жизнь дракона 500, атака 55, жизнь одного копейщика 10, атака 10, а количество копейщиков - 20. Лог боя 
		//для данного примера должен выглядеть так:
		...
		int kopatak = 10;
		int kopzdr = 10;
		while(drakon>(kop*kopatak)){
			System.out.println("Копейщики атакуют(урон " + kop*kopatak + ") - у дракона осталось " + (drakon - kop * kopatak) + " очков здоровья.");
			if((kop*kopzdr) < drakatak){
				System.out.println("Дракон атакует  и побеждает.");
				drakon = 500;
			}else {
			System.out.println("Дракон атакует(урон " + drakatak + ") - осталось " + ((int)(kop*kopzdr-drakatak)/10) + " копейщиков, один из которых ранен (осталось" + (kop*kopzdr-drakatak) + " очков здоровья).");
			drakon = drakon - (kop*kopatak);
			}
			kop++;
		}
