...
public class Shkurenko_5_2 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		if (mec == 3 && data >= 21 || mec == 4 && data <= 19){ //Овен
		...
		} else if (mec == 4 && data >= 20 || mec == 5 && data <= 20){//Телец
			System.out.println("Телец.\nОбщий гороскоп на эту неделю ");
			System.out.println("В начале неделе будьте предельно осторожны, так как в эти дни риск и признание будут ходить где-то рядом друг от друга. Остерегайтесь фальшивого блеска. В середине недели не стоит ввязываться в спор и идти на конфликт с начальством, за которым в любой ситуации остается последнее слово. Желательно избавиться от негативных эмоций - подключите друзей, которые находятся с Вами в одной лодке. Завершить неделю можно вечеринкой. Вы и Ваши товарищи сможете расслабиться и сбросить напряжение с помощью юмора и смеха.");
		} else if (mec == 5 && data >= 21 || mec == 6 && data <= 20){//Близнецы
			System.out.println("Близнецы.\nОбщий гороскоп на эту неделю ");
			System.out.println("На этой неделе откажитесь от комментариев там, где раньше непременно вставили бы словцо критики. Позвольте друзьям учиться на своих ошибках. Таким образом, Вы оградите себя от некоторых проблем. Лучшая стратегия на эти дни - позиция наблюдателя. Не ввязывайтесь в происходящее - Вы рискуете изменить свое мнение, сделав выводы из недостоверной информации. Выходные - прекрасное время, чтобы удовлетворить свои творческие потребности походом на концерт или в музей.");
		} else if (mec == 6 && data >= 21 || mec == 7 && data <= 22){//Рак
			System.out.println("Рак.\nОбщий гороскоп на эту неделю ");
			System.out.println("В начале недели позвольте себе небольшое проявление эгоцентризма - на данном этапе Вам это необходимо. Побалуйте себя, расслабьтесь - и почувствуете прилив энергии. Не расстраивайтесь, если вдруг возникнет чувство одиночества. Ничего страшного не произойдет, если Вы откажетесь от приглашения на обед; не ставьте себе рамки. В конце недели проведите время с друзьями. В эти дни не забывайте, что помогая нуждающимся, Вы помогаете в том числе и себе.");
		} else if (mec == 7 && data >= 23 || mec == 8 && data <= 22){//Лев
			System.out.println("Лев.\nОбщий гороскоп на эту неделю ");
			System.out.println("Романтические воспоминания будут поддерживать положительный настрой всю неделю. В первые дни вероятна поездка - ничего особенного, но она разожжет былую страсть к путешествиям. Вы точно знаете, какое место хотите посетить первым! Не все Вас могут понять, но Вы все еще продолжаете производить на окружающих положительное впечатление. В конце недели не позволяйте своей гордости проявляться в близких взаимоотношениях. Вам лучше слушать, сохранять спокойствие и самообладание.");
		} else if (mec == 8 && data >= 23 || mec == 9 && data <= 22){//Дева
			System.out.println("Дева.\nОбщий гороскоп на эту неделю ");
			System.out.println("Вы прекрасно знаете, что могли бы развить большую скорость, если бы Вас никто не отвлекал. Но возникает вопрос: в действительности ли Вам необходим такой стремительный разгон? На этой неделе стоит отказаться от спешки. Прислушайтесь к своим ощущениям. Вероятны небольшие конфликты из-за семейных обязанностей. Рискните и ответьте за последствия. Если Вы пока одиноки, ждите новой встречи на работе, которая может закончиться романом. Проявите искренность по отношению к окружающим.");
		} else if (mec == 9 && data >= 23 || mec == 10 && data <= 22){//Весы
			System.out.println("Весы.\nОбщий гороскоп на эту неделю ");
			System.out.println("Эту неделю посвятите поискам новых идей и полезного опыта. Подпитайте свой ум любопытной и интересной Вам информацией. В середине недели, даже если всплывут какие-то нелицеприятные воспоминания из прошлого, Вы увидите их в новом, лучшем свете. В пятницу свежий романтический бриз сдует всю Вашу грусть. Интеллектуальная близость принесет Вам вдохновение. Однако откажитесь от необходимости отменять запланированное ради начала нового проекта. На выходных внесите в окружающий мир капельку прекрасного.");
		} else if (mec == 10 && data >= 23 || mec == 11 && data <= 21){//Скорпион
			System.out.println("Скорпион.\nОбщий гороскоп на эту неделю ");
			System.out.println("Не умаляйте свои интуитивные способности. Не стоит следовать настоятельным советам окружающих и плыть по течению, ведь Вам прекрасно известно, что необходимо прислушиваться к голосу своего сердца. В течение недели амбиции вытеснят самообладание. Вы благополучно сотрудничаете с одной влиятельной персоной, но будьте начеку, так как она скора на обещания, но не всегда их выполняет. На выходных в Вашем доме может образоваться заряд негативной энергии. Прекрасным антидотом может стать перестановка или косметический ремонт.");
		} else if (mec == 11 && data >= 22 || mec == 12 && data <= 21){//Стрелец
			System.out.println("Стрелец.\nОбщий гороскоп на эту неделю ");
			System.out.println("Эта неделя наполнится теплотой и чувствами. Особенно трогательным станет момент, когда Ваш друг выразит Вам свое нежное отношение. Вероятно, появятся финансовые проблемы, которые дадут о себе знать в тот момент, когда придет время оплачивать очередную квитанцию. К концу недели появится некоторая эмоциональная усталость, но это не станет преградой для неплохого времяпровождения.");
		} else if (mec == 12 && data >= 22 || mec == 1 && data <= 19){//Козерог
			System.out.println("Козерог.\nОбщий гороскоп на эту неделю ");
			System.out.println("Ваш перфекционизм поможет Вам завершить начатое дело, четко придерживаясь плана. Дисциплина - ключ к успеху. К счастью, до четверга Вы уже разберетесь с заданием и добьетесь заслуженной похвалы. Такой заряд позитивной энергии подвигнет Вас на новые свершения. Творческое вдохновение проявите через танцы, пение или рисование.");
		} else if (mec == 1 && data >= 20 || mec == 2 && data <= 18){//Водолей
			System.out.println("Водолей.\nОбщий гороскоп на эту неделю ");
			System.out.println("Текущая неделя - кладезь возможностей. Самое невероятное может стать явью, поэтому полный вперед! Проведите время в компании друзей. Общаясь с ними, Вы совершенствуете свои коммуникативные навыки. Вы станете центром, который соединяет все точки и отображает общую картину в любом обществе. В середине недели сильный заряд энергии подвигнет Вас на новые открытия и заставит задуматься о серьезных переменах. Убедитесь, что у Вас в наличии есть четкий план действий, иначе в итоге будет сложно завершить начатые перестановки. К концу недели контролируйте траты - Ваша уверенность в достаточных запасах может оказаться обманчивой.");
		} else if (mec == 2 && data >= 19 || mec == 3 && data <= 20){//Рыбы
		...
		*/
		//15.Валера купил N куриц по M гривен за одну курицу. Каждая курица несёт по X яиц в неделю. Валера решил открыть свой бизнес и 
		//стал продавать яйца, которые несут курицы. За сколько дней окупятся куры, если продавать яйца по Z гривен за десяток 
		//(с учётом НДС, но без учёта сбора в ПФ)? Сроком жизни куриц, влажностью воздуха в курятнике, а также скоростью ветра и фазами луны можно пренебречь.
		...
		double den =(N*M)/((N/10*X*Z)/7+(((N/10*Z)/7)*0.2));
		System.out.println("Бизнес окупиться за " + den);
